...
var iTiempoTranscurrido=iPuntosObtenidos=0, iTiempoLimite=100, objPrimero;
var finalizo=false;
...
$(document).ready(function(){
	...
	var strCuadros=[1,2,3,4,5,6], iRepeticiones=6;
	
	//evento al hacer clic en los items de la lista
	$('ul li').live('click',function(){
		if(!finalizo && $(this).css('opacity')!=0){
			...
			if(objPrimero==undefined){
				...
				if(objPrimero.index()!=objSegundo.index()){
					...
					if(objPrimero.attr('rel')==objSegundo.attr('rel')){
						...
						$(objPrimero).stop(true,true).animate({opacity: 1}).delay(300).animate({opacity: 0});
						$(objSegundo).stop(true,true).animate({opacity: 1}).delay(300).animate({opacity: 0});
